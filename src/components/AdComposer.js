...
const PlatformIcon = ({ platform }) => {
  switch (platform) {
    ...
    case 'LinkedIn':
      return (
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" className="text-blue-700">
          <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z"/>
        </svg>
      );
    case 'Google Ads':
      return (
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" className="text-yellow-500">
...
export default function AdComposer() {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (user) {
        try {
          const accounts = await socialMediaService.getConnectedAccounts(user.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!isConnected) {
        throw new Error(`You don't have a connected ${platform} account. Please connect it in the settings.`);
      }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2">
            Media Upload {platformMediaSupport[platform].image && platformMediaSupport[platform].video ? 
              '(Images & Videos)' : 
              platformMediaSupport[platform].image ? '(Images Only)' : 
              platformMediaSupport[platform].video ? '(Videos Only)' : '(Not Supported)'}
          </label>
          
          {(platformMediaSupport[platform].image || platformMediaSupport[platform].video) && (
              ...
              {!mediaPreview ? (
                <div>
                  <input 
                    ...
                    accept={`${platformMediaSupport[platform].image ? 'image/*,' : ''}${platformMediaSupport[platform].video ? 'video/*' : ''}`}
                  />
                  <label 
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    {platformMediaSupport[platform].image && platformMediaSupport[platform].video ? 
                      ...
                      platformMediaSupport[platform].image ? 'Upload an image for your post' : 
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <textarea
              ...
              className={`block w-full bg-white border ${charCount.isOverLimit ? 'border-red-500' : charCount.isNearLimit ? 'border-yellow-500' : 'border-gray-300'} rounded py-3 px-4 mb-1 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-h-[150px]`}
              placeholder={`What's on your mind? (${charCount.limit} characters max)`}
            />
            <div className={`text-xs text-right ${charCount.isOverLimit ? 'text-red-500 font-bold' : charCount.isNearLimit ? 'text-yellow-600' : 'text-gray-500'}`}>
              {charCount.count} / {charCount.limit} characters
            </div>
            <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1">
              <div 
                className={`h-1.5 rounded-full ${charCount.isOverLimit ? 'bg-red-500' : charCount.isNearLimit ? 'bg-yellow-500' : 'bg-green-500'}`} 
        ...
        <div className="flex items-center justify-end">
          <button
            ...
            className={`bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded focus:outline-none focus:shadow-outline flex items-center ${isLoading || charCount.isOverLimit ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {isLoading ? (
              <>
